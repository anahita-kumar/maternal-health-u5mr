...
# Set start row for Excel import
start_row <- 17
...
df <- read_excel("01_rawdata/population_demographics.xlsx", 
                 sheet = "Projections",
                 skip = start_row - 1)
...
anc_clean <- read.csv("01_rawdata/fusion_file_1.csv") %>%
  clean_names() %>%
...
u5mr_clean <- read_xlsx("01_rawdata/country_tracking.xlsx") %>%
  select(
    iso3 = ISO3Code,
...
ggsave("03_outputs/coverage_plot.png", ggplot_obj, width = 8, height = 5)
...
report_lines <- c(
  ...
  "knitr::include_graphics('03_outputs/coverage_plot.png')",
  "```",
  "",
...
rmarkdown::render("report.Rmd", output_file = "03_outputs/final_report.html")
